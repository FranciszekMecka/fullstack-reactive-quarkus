...
import mecka.franciszek.task.Task;
...
public class UserService {
  ...
  @WithTransaction
  public Uni<User> update(User user, long id) {
    return findById(id)
      .chain(u -> {
        user.updatePassword(u.getPassword());
  ...
  public Uni<User> changePassword(String currentPassword, String newPassword) {
    return getCurrentUser()
      .chain(u -> {
        ...
        u.updatePassword(BcryptUtil.bcryptHash(newPassword));
  ...
  public Uni<Void> delete(long id) {
    return findById(id)
      .chain(u -> Uni.combine().all().unis(
            Task.delete("user.id", u.getId()),
            Project.delete("user.id", u.getId())
          ).asTuple()
          .replaceWith(u.delete())
      );
  }
